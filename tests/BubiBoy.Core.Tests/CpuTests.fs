...
let ``CGB STOP performs prepared speed switch without halting`` () =
    let bus = makeCgbBus [| 0x10uy; 0x00uy |] |> Bus.writeByte 0xFF4Dus 0x01uy
...
let ``LD A DE reads through bus`` () =
    let bus = makeBus [| 0x1Auy |] |> Bus.writeByte 0xC123us 0xA5uy
...
let ``LD A BC reads through bus`` () =
    let bus = makeBus [| 0x0Auy |] |> Bus.writeByte 0xC456us 0x6Auy
...
let ``XOR HL updates A from memory`` () =
    let bus = makeBus [| 0xAEuy |] |> Bus.writeByte 0xC020us 0x0Fuy
...
let ``LDH a8 A stores A in high IO page`` () =
    ...
    let cpu =
        { Cpu.initialState with
            Registers = { Cpu.initialRegisters with A = 0x91uy } }
...
let ``LD HL d16 and LD HLD A write through bus and decrement HL`` () =
    ...
    let cpu =
        { Cpu.initialState with
            Registers = { Cpu.initialRegisters with A = 0x77uy } }
...
let ``LD A HLI reads through bus and increments HL`` () =
    let bus = makeBus [| 0x2Auy |] |> Bus.writeByte 0xC020us 0x77uy
...
let ``LD A HLD reads through bus and decrements HL`` () =
    let bus = makeBus [| 0x3Auy |] |> Bus.writeByte 0xC020us 0x3Auy
...
let ``INC HL memory increments memory and updates flags`` () =
    let bus = makeBus [| 0x34uy |] |> Bus.writeByte 0xC010us 0x0Fuy
...
let ``DEC HL memory decrements memory and updates flags`` () =
    let bus = makeBus [| 0x35uy |] |> Bus.writeByte 0xC010us 0x10uy
...
let ``JR NZ branches only when zero flag is clear`` () =
    ...
    let clearZero =
        { Cpu.initialState with
            Registers = { Cpu.initialRegisters with F = 0uy } }
...
let ``LDH A a8 reads from high IO page`` () =
    let bus = makeBus [| 0xF0uy; 0x80uy |] |> Bus.writeByte 0xFF80us 0x5Auy
...
let ``LD A C reads from high IO page addressed by C`` () =
    let bus = makeBus [| 0xF2uy |] |> Bus.writeByte 0xFF80us 0xA6uy

    let cpu =
        { Cpu.initialState with
            Registers = { Cpu.initialRegisters with C = 0x80uy } }
...
let ``LD A a16 reads through bus`` () =
    let bus = makeBus [| 0xFAuy; 0x34uy; 0xC1uy |] |> Bus.writeByte 0xC134us 0xADuy
...
let ``LD B HL reads through bus`` () =
    let bus = makeBus [| 0x46uy |] |> Bus.writeByte 0xC044us 0x46uy
...
let ``LD C HL reads through bus`` () =
    let bus = makeBus [| 0x4Euy |] |> Bus.writeByte 0xC040us 0x4Euy
...
let ``LD D HL reads through bus`` () =
    let bus = makeBus [| 0x56uy |] |> Bus.writeByte 0xC042us 0x56uy
...
let ``LD E HL reads through bus`` () =
    let bus = makeBus [| 0x5Euy |] |> Bus.writeByte 0xC041us 0x5Euy
...
let ``LD L HL reads through bus`` () =
    let bus = makeBus [| 0x6Euy |] |> Bus.writeByte 0xC020us 0x6Euy
...
let ``LD A A preserves accumulator`` () =
    ...
    let cpu =
        { Cpu.initialState with
            Registers = { Cpu.initialRegisters with A = 0x7Fuy } }
...
let ``LD A HL reads through bus`` () =
    let bus = makeBus [| 0x7Euy |] |> Bus.writeByte 0xC043us 0x7Euy
...
let ``LD H HL reads through bus into H using original HL address`` () =
    let bus = makeBus [| 0x66uy |] |> Bus.writeByte 0xC020us 0x99uy
...
let ``ADD A d8 adds immediate into accumulator`` () =
    ...
    let cpu =
        { Cpu.initialState with
            Registers = { Cpu.initialRegisters with A = 0xFFuy } }
...
let ``ADD A HL adds memory into accumulator`` () =
    let bus = makeBus [| 0x86uy |] |> Bus.writeByte 0xC020us 0x01uy
...
let ``ADC A HL adds memory and carry into accumulator`` () =
    let bus = makeBus [| 0x8Euy |] |> Bus.writeByte 0xC020us 0x00uy
...
let ``SUB A register subtracts selected register from accumulator`` opcode =
    ...
    let expectedFlags =
        if opcode = 0x97uy then
            Cpu.ZeroFlag ||| Cpu.SubtractFlag
        else
            Cpu.SubtractFlag ||| Cpu.HalfCarryFlag
...
let ``SUB HL subtracts memory from accumulator`` () =
    let bus = makeBus [| 0x96uy |] |> Bus.writeByte 0xC020us 0x21uy
...
let ``SUB d8 subtracts immediate from accumulator`` () =
    ...
    let cpu =
        { Cpu.initialState with
            Registers = { Cpu.initialRegisters with A = 0x10uy } }
...
let ``SBC A register subtracts selected register and carry`` opcode =
    ...
    let expectedFlags =
        if opcode = 0x9Fuy then
            Cpu.SubtractFlag ||| Cpu.HalfCarryFlag ||| Cpu.CarryFlag
        else
            Cpu.SubtractFlag ||| Cpu.HalfCarryFlag
...
let ``SBC A HL subtracts memory and carry`` () =
    let bus = makeBus [| 0x9Euy |] |> Bus.writeByte 0xC020us 0x01uy
...
let ``CP HL compares accumulator with memory and preserves A`` () =
    let bus = makeBus [| 0xBEuy |] |> Bus.writeByte 0xC000us 0x20uy
...
let ``AND HL updates A and flags`` () =
    let bus = makeBus [| 0xA6uy |] |> Bus.writeByte 0xC020us 0x0Fuy
...
let ``OR HL updates A from memory and clears arithmetic flags`` () =
    let bus = makeBus [| 0xB6uy |] |> Bus.writeByte 0xC020us 0x06uy
    ...
    let cpu =
        { Cpu.initialState with
            Registers =
                { Cpu.initialRegisters with
                    A = 0x0Fuy
                    F = Cpu.SubtractFlag ||| Cpu.HalfCarryFlag } }
...
let ``SCF sets carry and preserves zero`` () =
    ...
    let cpu =
        { Cpu.initialState with
            Registers =
                { Cpu.initialRegisters with
                    F = Cpu.ZeroFlag ||| Cpu.SubtractFlag ||| Cpu.HalfCarryFlag } }
...
let ``CCF complements carry and preserves zero`` () =
    ...
    let cpu =
        { Cpu.initialState with
            Registers =
                { Cpu.initialRegisters with
                    F = Cpu.ZeroFlag ||| Cpu.CarryFlag ||| Cpu.SubtractFlag ||| Cpu.HalfCarryFlag } }
...
let ``CB 9E resets bit three through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x9Euy |] |> Bus.writeByte 0xC020us 0xFFuy
...
let ``CB A6 resets bit four through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xA6uy |] |> Bus.writeByte 0xC020us 0xFFuy
...
let ``CB AE resets bit five through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xAEuy |] |> Bus.writeByte 0xC020us 0xFFuy
...
let ``CB B6 resets bit six through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xB6uy |] |> Bus.writeByte 0xC020us 0xFFuy
...
let ``CB DE sets bit three through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xDEuy |] |> Bus.writeByte 0xC020us 0x00uy
...
let ``CB 96 resets bit two through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x96uy |] |> Bus.writeByte 0xC020us 0xFFuy
...
let ``CB 86 resets bit zero through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x86uy |] |> Bus.writeByte 0xC020us 0xFFuy
...
let ``CB 8E resets bit one through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x8Euy |] |> Bus.writeByte 0xC020us 0xFFuy
...
let ``CB E6 sets bit four through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xE6uy |] |> Bus.writeByte 0xC020us 0x00uy
...
let ``CB D6 sets bit two through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xD6uy |] |> Bus.writeByte 0xC020us 0x00uy
...
let ``CB CE sets bit one through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xCEuy |] |> Bus.writeByte 0xC020us 0x00uy
...
let ``CB C6 sets bit zero through HL`` () =
    let bus = makeBus [| 0xCBuy; 0xC6uy |] |> Bus.writeByte 0xC020us 0x00uy
...
let ``CB 36 swaps nibbles through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x36uy |] |> Bus.writeByte 0xC020us 0xF0uy
...
let ``CB 0E rotates HL memory right circular`` () =
    let bus = makeBus [| 0xCBuy; 0x0Euy |] |> Bus.writeByte 0xC020us 0x01uy
    ...
    let cpu =
        { Cpu.initialState with
            Registers =
                { Cpu.initialRegisters with
                    A = 0x02uy
                    F = Cpu.CarryFlag ||| Cpu.SubtractFlag } }
...
let ``CB 46 tests bit zero through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x46uy |] |> Bus.writeByte 0xC020us 0x01uy
...
let ``CB 4E tests bit one through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x4Euy |] |> Bus.writeByte 0xC020us 0x00uy
...
let ``CB 56 tests bit two through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x56uy |] |> Bus.writeByte 0xC020us 0x04uy
...
let ``CB 5E tests bit three through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x5Euy |] |> Bus.writeByte 0xC020us 0x08uy
...
let ``CB 66 tests bit four through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x66uy |] |> Bus.writeByte 0xC020us 0x10uy
...
let ``CB 6E tests bit five through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x6Euy |] |> Bus.writeByte 0xC020us 0x20uy
...
let ``CB 76 tests bit six through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x76uy |] |> Bus.writeByte 0xC020us 0x40uy
...
let ``CB 7E tests bit seven through HL`` () =
    let bus = makeBus [| 0xCBuy; 0x7Euy |] |> Bus.writeByte 0xC020us 0x80uy
...
let ``generic CB SRA through HL preserves sign bit`` () =
    let bus = makeBus [| 0xCBuy; 0x2Euy |] |> Bus.writeByte 0xC020us 0x81uy
